...
export function DashboardContent() {
  ...
  const { isOpen: marketIsOpen, status: marketStatusText, reason: marketReason, loading: marketLoading } = useMarketStatus()

  const loadStocks = useCallback(async () => {
    try {
      const stocks = await fetchAllStocks()
      const sorted = [...stocks].sort((a: any, b: any) => parseFloat(b.change) - parseFloat(a.change))
  ...
  // Always fetch once on mount (so user sees data on login)
  useEffect(() => {
    loadStocks()
    loadIndices()
  }, [loadStocks, loadIndices])

  // Poll stocks/indices only when market is open
  useEffect(() => {
    if (!marketIsOpen || marketLoading) return
    const intervalId = setInterval(() => {
      loadStocks()
      loadIndices()
    }, 10000)
    return () => clearInterval(intervalId)
  }, [marketIsOpen, marketLoading, loadStocks, loadIndices])

  const displayStatus = marketLoading ? "Loading" : marketIsOpen ? "Live" : "Closed"
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-2">
        <div className="flex items-center gap-3">
          ...
          <Badge variant={displayStatus === "Live" ? "default" : "secondary"} className="h-6 gap-1 px-2.5 outline-none font-medium cursor-default" title={!marketIsOpen && marketReason ? marketReason : undefined}>
